...
def combine_ranges(sensor_ranges):
    while (True):
...
sensor_ranges = set([sensor.beacon_x_range_exclusive(y) for sensor in sensors])
sensor_ranges.remove(None)
sensor_ranges = combine_ranges(sensor_ranges)  # combine all possible sensors

sum = 0
for sensor_range in sensor_ranges:
    sum += len_range(sensor_range)

beacons = set([sensor.beacon for sensor in sensors])
new_beacons = []
for beacon in beacons:
    for sensor_range in sensor_ranges:
        if beacon.y == y and beacon.x >= sensor_range[0] and beacon.x < sensor_range[1]:
            new_beacons.append(beacon)

new_beacons = set(new_beacons)
print(sum - len(new_beacons))
